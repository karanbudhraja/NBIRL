...
% karan making manual 4x4 mdp

% select 3 cells at random
%goal_states = randperm(mdp_params.n^2,1);
%goal_states = randperm(mdp_params.n^2,2);
%goal_states = randperm(mdp_params.n^2,3);
%goal_states = randperm(mdp_params.n^2,4);

goal_states = randperm(mdp_params.n^2,64);

goal_states_y = ceil(goal_states/mdp_params.n);
goal_states_x = goal_states - (goal_states_y - 1)*mdp_params.n;

% check neighbors at each goal state
for index = 1:length(goal_states)
    goal_x = goal_states_x(index);
    goal_y = goal_states_y(index);

    if(goal_x == 0)
        goal_x = mdp_params.n;
    end

    r((goal_y-1)*mdp_params.n+goal_x,1) = 100;

    if(goal_x + 1 <= mdp_params.n)
        r((goal_y-1)*mdp_params.n+goal_x+1,5) = 100;
    end

    if(goal_x - 1 > 0)
        r((goal_y-1)*mdp_params.n+goal_x-1,3) = 100;
    end

    if(goal_y + 1 <= mdp_params.n)
        r((goal_y+1-1)*mdp_params.n+goal_x,4) = 100;
    end

    if(goal_y - 1 > 0)
        r((goal_y-1-1)*mdp_params.n+goal_x,2) = 100;
    end
end

%{
r((3-1)*mdp_params.n+1,:) = [0 100 0 0 0];
r((3-1)*mdp_params.n+2,:) = [0 0 0 0 100];

r((1-1)*mdp_params.n+2,:) = [0 100 0 0 0];
r((2-1)*mdp_params.n+1,:) = [0 0 100 0 0];
r((2-1)*mdp_params.n+3,:) = [0 0 0 0 100];
r((3-1)*mdp_params.n+2,:) = [0 0 0 100 0];

r((2-1)*mdp_params.n+4,:) = [0 100 0 0 0];
r((3-1)*mdp_params.n+3,:) = [0 0 100 0 0];
r((4-1)*mdp_params.n+4,:) = [0 0 0 100 0];

r((4-1)*mdp_params.n+1,1) = 100;
r((2-1)*mdp_params.n+2,1) = 100;
r((3-1)*mdp_params.n+4,1) = 100;
%}

%{
r((2-1)*mdp_params.n+2,:) = [0 100 100 0 0];
r((3-1)*mdp_params.n+1,:) = [0 0 100 0 0];
r((4-1)*mdp_params.n+2,:) = [0 0 0 100 0];

r((1-1)*mdp_params.n+3,:) = [0 100 0 0 0];
r((2-1)*mdp_params.n+4,:) = [0 0 0 0 100];

r((3-1)*mdp_params.n+4,:) = [0 0 0 0 100];
r((4-1)*mdp_params.n+3,:) = [0 0 0 100 0];

r((2-1)*mdp_params.n+3,1) = 100;
r((3-1)*mdp_params.n+2,1) = 100;
r((3-1)*mdp_params.n+3,1) = 100;
%}
