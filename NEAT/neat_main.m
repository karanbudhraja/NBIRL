...
function [best_policy,total_time_taken] = neat_main(algorithm_params)

% algorithm_params - parameters of the specified algorithm:
%   population_size - population of genomes used
%   max_generations - number of NEAT generations
%	number_input_nodes - number of input nodes
...
clearvars -except algorithm_params;
tic;
%list of parameters
  
  %parameters of main file
    %maxgeneration=200; %maximum number of generations for generational loop    
	maxgeneration = algorithm_params.max_generations; % now a parameter
	...
	% karan changed save flag to 0
	save_flag=0; %if set to 1, will save population, generation, innovation_record and species_record to neatsave.mat at every generation (default option)
  ...
  %parameters initial population
	...
	population_size = algorithm_params.population_size;	% now a parameter
	number_input_nodes = algorithm_params.number_input_nodes;	% now a parameter
...
while generation<maxgeneration & flag_solution==0
   ...
   %population=karan_experiment(population);
   [population, best_fitness, best_output]=gridworld_experiment(population, number_input_nodes, algorithm_params.state_features, algorithm_params.example_state_actions);
...
%   if maximale_fitness>15.6025
   ...
   % check if entire grid has been matched
   if maximale_fitness == size(algorithm_params.state_features,1)
      flag_solution=1;
   end
   
   % karan commenting plots 
   if(algorithm_params.verbosity > 0)   
	   subplot(2,2,1);
...
%
% karan: now we need to return the best policy
%

total_time_taken = toc;

% convert output to a policy
best_policy = values_to_policy(best_output);
