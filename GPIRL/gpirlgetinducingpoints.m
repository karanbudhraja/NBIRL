...
elseif strcmp(algorithm_params.inducing_pts,'examplesplus'),
    ...
    if length(s_u) < algorithm_params.inducing_pts_count,
        ...
        % karan adding patch for smaller grids
        % if inducing point count is out of bounds as index, lower it
        if((algorithm_params.inducing_pts_count-length(s_u)) > length(other_states))
            algorithm_params.inducing_pts_count = length(other_states) + length(s_u);
        end
