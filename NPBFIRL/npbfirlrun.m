...
% karan adding for quick simulation
%alg_params.max_iters = 10;
...
% Set random seed.
if(alg_params.seed >= 0)
	rand('seed',alg_params.seed);
end
...
% karan: calculate neat estimate
neat.r = zeros(mdp_data.states, 1);

if(strcmp(alg_params.policy_type, 'neat') == 1)
    neat_input = zeros(mdp_data.states, alg_params.max_iters - nsamples);

    for i = nsamples + 1:alg_params.max_iters
        current_product = hst.X{i}.F*hst.X{i}.w;
        for index = 1:size(current_product,1)
	    neat_input(index,i) = current_product(index);
       end
    end

    % karan debug
    [best_output,total_time_taken] = npbfirlneat(struct('population_size',alg_params.population_size,'max_generations',alg_params.max_generations,'number_input_nodes',size(neat_input,2),...
		   'neat_input',neat_input,'mdp_data',mdp_data,'verbosity',verbosity),...
	    alg_params, nsamples, Eo);
    neat.r = best_output;
...
if(strcmp(alg_params.policy_type, 'mean') == 1)
	irl_result = struct('time', endTm, 'map', map, 'mean', mean, 'hst', hst, 'p', mean.soln.p);
elseif(strcmp(alg_params.policy_type, 'neat') == 1)
        irl_result = struct('time', endTm, 'map', map, 'mean', mean, 'hst', hst, 'p', neat.soln.p);
else
        % default to map
	irl_result = struct('time', endTm, 'map', map, 'mean', mean, 'hst', hst, 'p', map.soln.p);
end
